...
def train(args):
    ...
    if args.do_train:
        ...
        custom_model = CustomModel(model, args.custom_model_d, codebert_output_dim=768 * args.window_size,
                                   input_dim=args.window_size)
        labels = torch.tensor(tokenized_hf['train']['masks'])
        dataset = TripletDataset(torch.tensor(tokenized_hf['train']['input_ids']),
                                 m_labels=torch.tensor(tokenized_hf['train']['m_labels']), labels=labels,
                                 dataset_name="train")
            ...
            for step in tqdm(range(len(dataset)), desc="Steps"):
                ...
                if step > 0 and step % INTERVAL == 0:
                    if args.output_dir is not None:
                        torch.save(custom_model, args.output_dir + "/model_intermediary" + (
                            "_classification" if args.use_classifier else "") + str(count) + ".pth")
                    count += 1
                if not args.use_classifier:
                    ...
                    target = torch.nn.functional.one_hot(torch.tensor([masked_label_position]),
                                                         args.custom_model_d).squeeze(0).double()
        ...
        if not args.use_classifier:
            ...
            if not os.path.isfile("models/" + args.use_typespace):
                ...
                space, computed_mapped_labels_train = create_type_space(custom_model, torch.tensor(
                    tokenized_hf['train']['input_ids']), torch.tensor(tokenized_hf['train']['m_labels']),
                                                                        torch.tensor(tokenized_hf['train']['masks']))
